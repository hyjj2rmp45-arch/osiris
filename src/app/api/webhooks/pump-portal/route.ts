...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!isAllowedIP(clientIP)) {
      await notifyNtfy('OSIRIS Error', `PumpPortal webhook error: IP ${clientIP} not in allowlist`, 'error,pump-portal', ctx);
      return NextResponse.json({ error: 'IP not allowed' }, { status: 403 });
    }

    const signature = request.headers.get('x-signature') || request.headers.get('x-pump-signature') || '';
    if (!signature) {
      await notifyNtfy('OSIRIS Error', 'PumpPortal webhook error: Missing signature header', 'error,pump-portal', ctx);
      return NextResponse.json({ error: 'Missing signature header' }, { status: 400 });
    }

    const rawBody = await request.text();
    if (!verifySignature(rawBody, signature)) {
      await notifyNtfy('OSIRIS Error', 'PumpPortal webhook error: Signature verification failed', 'error,pump-portal', ctx);
    ...
    try {
      ...
      await notifyNtfy('OSIRIS Error', `PumpPortal webhook error: JSON parse error: ${msg}`, 'error,pump-portal', ctx);
    ...
    if (!parsed.success) {
      ...
      await notifyNtfy('OSIRIS Error', `PumpPortal webhook error: Payload validation failed: ${details}`, 'error,pump-portal', ctx);
    ...
    if (circuitBreaker.isEngaged()) {
      await notifyNtfy('OSIRIS Error', 'PumpPortal circuit breaker half-open', 'error,circuit-breaker', ctx);
    ...
    await notifyNtfy('OSIRIS Error', `PumpPortal webhook error: ${message}`, 'error,pump-portal', ctx);
